...
public class PickUpAndHold : MonoBehaviour
{
    ...
    // Audio clip for picking up shields
    ...
    {
        if (collider2D.gameObject.CompareTag("Collect"))
        {
   ...
   private void OnCollisionExit2D(Collision2D collision)
    {
        ...
        if(collision.gameObject.CompareTag("Rock"))  //Changed music script!
        {
            AudioSource audioSource = GetComponent<AudioSource>();
            audioSource.Stop();
            audioSource.clip = rockMusic;
            audioSource.Play(); // --still when bouncing around the collider, the music stops and starts ggrrr.

               // audioSource.PlayOneShot(rockMusic);       works too. 
        }
